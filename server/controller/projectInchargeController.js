...
getActiveEnginrres: async(req, res)=>{
    try{
   const activeEngineers= await SiteEngineer.find({isActive: true});
...
// Get all Site Engineers
 getSiteEngineers : async (req, res) => {
    try {
        const siteEngineers = await SiteEngineer.find()
            .populate("assignedPlaza", "plazaName") // Assuming you want to populate plaza details (adjust field name as needed)
            .populate("assignedBy", "firstName lastName username"); // Populate assignedBy with specific fields
